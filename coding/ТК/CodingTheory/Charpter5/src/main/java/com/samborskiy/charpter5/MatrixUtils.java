...
public class MatrixUtils {
    ...
    /**
     * Generates random sequence n-th length.
     */
    public static DoubleMatrix generateSequence(int n) {
        Random random = new Random();
        double[][] sequence = new double[1][n];
        for (int i = 0; i < n; i++) {
            sequence[0][i] = random.nextInt(2);
        }
        return new DoubleMatrix(sequence);
    }
}
